...
function App() {
  ...
  const fetchExercises = async () => {
    return await fetch('https://127.0.0.1:3002/exercises/' + group).then(async response => {
      if (!response.ok) {
  ...
  const fetchPrograms = async () => {
    return await fetch('https://127.0.0.1:3002/programs/' + category).then(async response => {
      if (!response.ok) {
        throw new Error('Ошибка сервера: ' + response.status);
      } else {
        return response.json();
